...
class ActionLayer(BaseActionLayer):
    ...
    def _inconsistent_effects(self, actionA, actionB):
        ...
        for fx_a in actionA.effects:
            for fx_b in actionB.effects:
                if fx_a == ~fx_b:
                    return True
    ...
    def _interference(self, actionA, actionB):
        ...
        """
        # TODO: implement this function
        for pr_a in actionA.preconditions:
            for fx_b in actionB.effects:
                if pr_a == ~fx_b:
                    return True

        for pr_b in actionB.preconditions:
            for fx_a in actionA.effects:
                if pr_b == ~fx_a:
                    return True
    ...
    def _competing_needs(self, actionA, actionB):
        ...
        for pr_a in actionA.preconditions:
            for pr_b in actionB.preconditions:
                if self.parent_layer.is_mutex(pr_a, pr_b):
                    return True
...
class LiteralLayer(BaseLiteralLayer):
    ...
    def _inconsistent_support(self, literalA, literalB):
        ...
        for action in self.parent_layer:
            is_literal_a = literalA in action.effects
            if is_literal_a:
                parent_actions_a.append(action)

            is_literal_b = literalB in action.effects
            if is_literal_b:
                parent_actions_b.append(action)

        return all(self.parent_layer.is_mutex(actionA, actionB) for actionB in parent_actions_b for actionA in parent_actions_a)
